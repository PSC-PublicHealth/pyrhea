...
def doubleSumBugWorkaround(tsDF):
    # Construct a dict that maps the value of the 'ward' column (which is the sum of the ward IDs and thus
    # follows a known pattern) to the number of wards.
    wardCtD = {}
    for i in xrange(40):
        wardCtD[sum(range(i+1))] = i+1

    def howManyWards(col):
        return [wardCtD[val] for val in col.tolist()]
    filtDF = tsDF[['ward']].copy()
    filtDF['nWards'] = filtDF[['ward']].apply(howManyWards, axis=0, raw=True)

    for col in tsDF.columns:
        if col in ['newColonized', 'contactPrecautionDays', 'creBundlesHandedOut', 'creSwabsUsed',
                  'arrivals', 'localtierdepartures', 'creArrivals',
                  'passiveCPDays', 'swabCPDays', 'otherCPDays', 'xdroCPDays',
                  'newPatientsOnCP']:
            filtDF[col] = tsDF[col]/filtDF['nWards']
        elif col == 'ward':
            pass
        else:
            filtDF[col] = tsDF[col]
...
def main():
    ...
    parser.add_option('--double_sum_bug_workaround', action='store_true', default=False,
                      help=('This activates code to correct for an error in output files'
                            'generated by pyRHEA 84a05ee to 2fe65fa7 (Nov 2018 - Jan 2019)'
                            ' If applied to a dataset which does not include the bug, this'
                            ' tool will almost certainly crash'))
    ...
    if opts.double_sum_bug_workaround:
        ...
        tsDF = doubleSumBugWorkaround(tsDF)
